...
class Test_heat_index:
    ...
    def setUpClass(self) -> None:
        # set up ground truths
        self.__class__.ncl_gt_1 = [
            137.36142, 135.86795, 104.684456, 131.25621, 105.39449, 79.78999,
            83.57511, 59.965, 30.
        ]
        self.__class__.ncl_gt_2 = [
            68.585, 76.13114, 75.12854, 99.43573, 104.93261, 93.73293,
            104.328705, 123.23398, 150.34001, 106.87023
        ]

        self.__class__.t1 = np.array([104, 100, 92, 92, 86, 80, 80, 60, 30])
        self.__class__.rh1 = np.array([55, 65, 60, 90, 90, 40, 75, 90, 50])

        self.__class__.t2 = np.array(
            [70, 75, 80, 85, 90, 95, 100, 105, 110, 115])
        self.__class__.rh2 = np.array([10, 75, 15, 80, 65, 25, 30, 40, 50, 5])
...
class Test_relhum:
    ...
    def setUpClass(self) -> None:
        ...
        self.__class__.rh_gt_1 = 46.4

        self.__class__.rh_gt_2 = [
...
class Test_actual_saturation_vapor_pressure:
    ...
    def setUpClass(self) -> None:
        ...
        # set up ground truths
        self.__class__.ncl_gt = np.asarray(ncl_xr_gt)

        self.__class__.temp_gt = np.arange(1, 101, 1)
...
class Test_max_daylight:
    ...
    def setUpClass(self) -> None:
        ...
        self.__class__.jday_gt = np.linspace(1, 365, num=365)
        self.__class__.lat_gt = np.linspace(-66, 66, num=133)
...
class Test_psychrometric_constant:
    ...
    def setUpClass(self) -> None:
        ...
        self.__class__.pressure_gt = np.arange(1, 101, 1)
...
class Test_Delta_Pressure:
    ...
    @pytest.fixture(autouse=True, scope="class")
    def setUpClass(self) -> None:
        self.__class__.pressure_lev = np.array([1, 5, 100, 1000])
        self.__class__.pressure_lev_da = xr.DataArray(
            self.__class__.pressure_lev)
        self.__class__.pressure_lev_da.attrs = {
            "long name": "pressure level",
            "units": "hPa",
            "direction": "descending"
        }

        self.__class__.surface_pressure_scalar = 1018
        self.__class__.surface_pressure_1D = np.array([1018, 1019])
        self.__class__.surface_pressure_2D = np.array([[1018, 1019],
                                                       [1017, 1019.5]])
        self.__class__.surface_pressure_3D = np.array([[[1018, 1019],
                                                        [1017, 1019.5]],
                                                       [[1019, 1020],
                                                        [1018, 1020.5]]])
        ...
        self.__class__.surface_pressure_3D_da = xr.DataArray(
            self.__class__.surface_pressure_3D,
            coords=coords,
            dims=dims,
            attrs=attrs)
