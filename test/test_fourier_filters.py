...
class Test_Fourier_One_Bands_Pass:
    ...
    def test_one_low_pass(self) -> None:
        t_expected_result = (np.sin(self.t * m.tau) / 0.1 +
                             np.sin(2 * self.t * m.tau) / 0.2 +
                             np.sin(5 * self.t * m.tau) / 0.5 +
                             np.sin(10 * self.t * m.tau))
    ...
    def test_one_band_pass(self) -> None:
        t_expected_result = (np.sin(5 * self.t * m.tau) / 0.5 +
                             np.sin(10 * self.t * m.tau) +
...
class Test_Fourier_Two_Bands_Pass:
    ...
    def test_two_band_block(self) -> None:
        t_expected_result = (np.sin(self.t * m.tau) / 0.1 +
                             ...
                             np.sin(50 * self.t * m.tau) / 5 +
                             np.sin(100 * self.t * m.tau) / 10)
        t_result = fourier_band_block(self.t_data,
                                      self.freq,
                                      3,
                                      30,
                                      time_axis=0)
