...
class TestDeltaPressure():
    ...
    def test_deltapressure_1D(self):
        ...
        delta_pressure = _calc_deltapressure_1D(self.pressure_lev, self.surface_pressure_scalar)
        assert sum(delta_pressure) == self.surface_pressure_scalar - pressure_top

    def test_missing_surface_pressure_warning(self):
        surface_pressure_missing = NaN
        with self.assertWarns(Warning):
            delta_pressure = _calc_deltapressure_1D(self.pressure_lev, surface_pressure_missing)

    def test_negative_pressure_warning(self):
        pressure_lev_negative = self.pressure_lev.copy()
        pressure_lev_negative[0] = -5
        with self.assertWarns(Warning):
            delta_pressure = _calc_deltapressure_1D(pressure_lev_negative, self.surface_pressure_scalar)
        
    def test_relative_pressure_warning(self):
        surface_pressure_low = 0.5
        with self.assertWarns(Warning):
            delta_pressure = _calc_deltapressure_1D(self.pressure_lev, surface_pressure_low)
        
    def test_convert_numpy_error(self):
        surface_pressure_long = 40*2*10**30
        self.assertRaises(AttributeError, calc_deltapressure(self.pressure_lev, surface_pressure_long))

        pressure_lev_long = 40*2*10**30
        self.assertRaises(AttributeError, calc_deltapressure(pressure_lev_long, self.surface_pressure_1D))
    
    def test_4_dimensions(self):
        surface_pressure_4D = [[[[1018, 1019], [1017, 1019.5]], [[1019, 1020], [1018, 1020.5]]], [[[1018, 1019], [1017, 1019.5]], [[1019, 1020], [1018, 1020.5]]]]
        with self.assertWarns(Warning):
            delta_pressure = calc_deltapressure(self.pressure_lev, surface_pressure_4D)
                
    def test_output_type(self):
        delta_pressure_da = calc_deltapressure(self.pressure_lev_da, self.surface_pressure_3D_da)
        assert type(delta_pressure_da) == xr.DataArray
        
        delta_pressure_np = calc_deltapressure(self.pressure_lev, self.surface_pressure_3D)
        assert type(delta_pressure_np) == np.Array
    
    def test_output_dimensions(self):
        delta_pressure_scalar = calc_deltapressure(self.pressure_lev, self.surface_pressure_scalar)
        assert delta_pressure_scalar.shape == (8)
        
        delta_pressure_1D = calc_deltapressure(self.pressure_lev, self.surface_pressure_1D)
        assert delta_pressure_1D.shape == (2, 8)

        delta_pressure_2D = calc_deltapressure(self.pressure_lev, self.surface_pressure_2D)
        assert delta_pressure_2D.shape == (2, 2, 8)

        delta_pressure_3D = calc_deltapressure(self.pressure_lev, self.surface_pressure_3D)
        assert delta_pressure_3D.shape == (2, 2, 2, 8)
    
    def test_output_attrs(self):
        delta_pressure_da = calc_deltapressure(self.pressure_lev_da, self.surface_pressure_3D_da)
        assert delta_pressure_da.attrs.pop("long name") == self.pressure_lev_da.attrs.pop("long name")
