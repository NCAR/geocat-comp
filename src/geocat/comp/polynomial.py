...
def _get_missing_value(data: xr.DataArray, args: dict) -> Any:
    """
    Attempts to return the missing_value from data.

    data -- data with possible missing_value or _FillValue property (xr.DataArray)
    args -- dictionary that may contain the name `missing_value` (dict)

    returns -- missing_value if found, or np.nan
    """
